  ...
  return (
    <div
      className={`flex h-16 items-center justify-center border-b-[1px] border-b-richblack-700 ${
        location.pathname !== "/" ? "bg-[#2f02d1]" : "bg-[#2f02d1]"
      } transition-all duration-200 `}
    >
      <div className="flex w-11/12 max-w-maxContent items-center justify-between ">
        {/* Logo */}
        <Link to="/">
          <img src={logo} alt="Logo" width={150} height={25} loading="lazy" className="mt-2" />
        </Link>
        {/* Navigation links */}
        ...
        <div className="hidden items-center gap-x-4 md:flex">
          ...
          {token === null && (
            <Link to="/login">
              <button className="rounded-[8px] border border-richblack-700 bg-yellow-50 px-[12px] py-[8px] text-richblack-800 hover:scale-105 transition transition-[all 500ms ease-in] ">
                Log in
              </button>
            </Link>
